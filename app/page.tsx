...
export default function Home() {
  ...
  useEffect(() => {
    const loadedItems = loadListings();
    setItems(loadedItems);
    setSaved(loadSaved());
    setMounted(true);
    const sharedId = new URLSearchParams(window.location.search).get("item");
    if (sharedId) {
      const sharedItem = loadedItems.find((item) => item.id === sharedId);
      if (sharedItem) setSelected(sharedItem);
    }
  }, []);
  ...
  const toggleSaved = (id: string) => {
    ...
    const next = wasSaved ? saved.filter((value) => value !== id) : [...saved, id];
    setSaved(next);
    ...
    <main id="top" className="min-h-screen overflow-hidden">
      <header className="sticky top-0 z-40 border-b border-[var(--line)] bg-[#f8f6f0]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
          <a href="#top" className="flex items-center gap-3" aria-label="GiveBack home"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[var(--ink)] text-white shadow-sm"><span className="text-lg font-black">GB</span></span><span><strong className="block text-lg tracking-tight">GiveBack</strong><span className="text-xs font-medium text-[var(--muted)]">good things, second chances</span></span></a>
          <nav className="hidden items-center gap-8 text-sm font-bold md:flex" aria-label="Main navigation"><a href="#discover" className="transition hover:text-[var(--leaf)]">Discover</a><a href="#how" className="transition hover:text-[var(--leaf)]">How it works</a><a href="#safety" className="transition hover:text-[var(--leaf)]">Safety</a></nav>
      ...
      <section className="relative mx-auto max-w-7xl px-5 pb-16 pt-14 lg:px-8 lg:pb-24 lg:pt-20">
        ...
        <div className="grid items-center gap-12 lg:grid-cols-[1.05fr_.95fr]">
          <div className="relative"><div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#cdd9c5] bg-[#edf3e8] px-4 py-2 text-xs font-black uppercase tracking-[.12em] text-[var(--leaf-dark)]"><Icon name="spark" /> Useful things deserve another chance</div><h1 className="max-w-4xl text-5xl font-black leading-[.98] tracking-[-.055em] sm:text-6xl lg:text-7xl">Give what you no longer need. <span className="text-[var(--leaf)]">Find what you do.</span></h1><p className="mt-7 max-w-2xl text-lg leading-8 text-[var(--muted)]">GiveBack is a simple community board for passing useful things forward. No price tags. No pressure. Just better use for things that still have life in them.</p><div className="mt-8 flex flex-wrap gap-3"><a href="#discover" className="inline-flex items-center gap-2 rounded-full bg-[var(--ink)] px-6 py-3.5 font-black text-white transition hover:-translate-y-0.5">Explore items <Icon name="arrow" /></a><button onClick={() => setShowGive(true)} className="rounded-full border border-[var(--line)] bg-white px-6 py-3.5 font-black transition hover:bg-[#f3f0e8]">List something useful</button></div><div className="mt-9 flex flex-wrap gap-x-7 gap-y-3 text-sm font-bold text-[var(--muted)]"><span className="inline-flex items-center gap-2"><Icon name="check" /> Free to give</span><span className="inline-flex items-center gap-2"><Icon name="check" /> Broad locations</span><span className="inline-flex items-center gap-2"><Icon name="check" /> Privacy-minded</span></div></div>
          <div className="relative rounded-[32px] bg-[var(--ink)] p-3 shadow-2xl sm:p-5"><div className="rounded-[25px] bg-[#f5f1e8] p-5 text-[var(--ink)] sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.16em] text-[var(--leaf)]">A better handoff</p><h2 className="mt-1 text-2xl font-black">The thing you forgot about could help someone today.</h2></div><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-[#dfe9d7] text-[var(--leaf-dark)]"><Icon name="heart" /></span></div><div className="mt-7 space-y-3"><div className="rounded-2xl border border-[var(--line)] bg-white p-4"><div className="flex items-center gap-3"><span className="text-3xl">📚</span><div className="min-w-0"><p className="truncate text-sm font-black">Form 4 Science Books</p><p className="mt-1 text-xs text-[var(--muted)]">Nearby · Good condition</p></div><span className="ml-auto rounded-full bg-[#e7efe1] px-2.5 py-1 text-xs font-black text-[var(--leaf-dark)]">free</span></div></div><div className="ml-8 rounded-2xl bg-[#e7efe1] p-4"><p className="text-sm font-black text-[var(--leaf-dark)]">Someone gets value.</p><p className="mt-1 text-xs leading-5 text-[var(--leaf-dark)]">You get the space back. The item gets another chapter.</p></div></div><p className="mt-6 text-xs font-semibold leading-5 text-[var(--muted)]">GiveBack uses broad areas rather than public home addresses. Always arrange handoffs safely.</p></div></div>
        </div>
      </section>

      <section id="discover" className="border-y border-[var(--line)] bg-[#ebe6da]/70"><div className="mx-auto max-w-7xl px-5 py-14 lg:px-8 lg:py-20"><div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><p className="text-xs font-black uppercase tracking-[.18em] text-[var(--leaf)]">Discover</p><h2 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">Useful things, waiting for a new home.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-[var(--muted)]">Search, filter and save. Your own listings and saves are stored locally in this browser.</p></div><div className="relative w-full lg:max-w-md"><label htmlFor="search" className="sr-only">Search items</label><div className="pointer-events-none absolute left-4 top-3.5 text-[var(--muted)]"><Icon name="search" /></div><input id="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search books, lamps, sports..." className="w-full rounded-2xl border border-[var(--line)] bg-white py-3.5 pl-12 pr-4 text-sm font-medium outline-none transition focus:border-[var(--leaf)] focus:ring-2 focus:ring-[#dfe9d7]" /></div></div><div className="mt-7 flex gap-2 overflow-x-auto pb-2" aria-label="Categories">{categories.map((itemCategory) => <button key={itemCategory} onClick={() => setCategory(itemCategory)} aria-pressed={category === itemCategory} className={`shrink-0 rounded-full px-4 py-2.5 text-sm font-black transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--leaf)] ${category === itemCategory ? "bg-[var(--ink)] text-white" : "border border-[var(--line)] bg-white/80 hover:bg-white"}`}>{itemCategory}</button>)}</div><div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm font-bold text-[var(--muted)]">{filtered.length} {filtered.length === 1 ? "item" : "items"}{showSavedOnly ? " saved" : " available to explore"}</p><div className="flex flex-wrap gap-2"><button onClick={() => setShowSavedOnly((value) => !value)} aria-pressed={showSavedOnly} className={`rounded-full px-4 py-2 text-sm font-black ${showSavedOnly ? "bg-[var(--leaf-dark)] text-white" : "border border-[var(--line)] bg-white"}`}>♥ Saved {mounted && saved.length > 0 ? `(${saved.length})` : ""}</button><label className="sr-only" htmlFor="sort">Sort items</label><select id="sort" value={sort} onChange={(event) => setSort(event.target.value as SortMode)} className="rounded-full border border-[var(--line)] bg-white px-4 py-2 text-sm font-bold outline-none"><option value="recent">Newest first</option><option value="nearby">Nearest first</option><option value="title">A–Z</option></select></div></div>{filtered.length > 0 ? <div className="mt-5 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{filtered.map((item) => <article key={item.id} className="group overflow-hidden rounded-[26px] border border-[var(--line)] bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg"><button className="w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--leaf)]" onClick={() => setSelected(item)} aria-label={`View ${item.title}`}><div className="relative flex h-48 items-center justify-center bg-[#ded9cc] text-7xl"><span aria-hidden="true">{item.emoji}</span><span className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1 text-xs font-black text-[var(--ink)]">{item.tag}</span><span className="absolute right-4 top-4 rounded-full bg-white/90 px-3 py-1 text-xs font-bold text-[var(--muted)]">{formatAge(item.createdAt)}</span></div><div className="p-5"><div className="flex items-center justify-between gap-3"><span className="rounded-full bg-[#e9f0e4] px-2.5 py-1 text-xs font-black text-[var(--leaf-dark)]">{item.condition}</span><span className="text-xs font-bold text-[var(--muted)]">{item.category}</span></div><h3 className="mt-4 text-lg font-black tracking-tight">{item.title}</h3><p className="mt-1 line-clamp-2 text-sm leading-6 text-[var(--muted)]">{item.note}</p><div className="mt-5 flex items-center gap-2 text-xs font-bold text-[var(--muted)]"><Icon name="location" /> {item.area}</div></div></button><div className="flex items-center justify-between border-t border-[var(--line)] px-5 py-3"><button onClick={() => toggleSaved(item.id)} aria-label={saved.includes(item.id) ? `Remove ${item.title} from saved` : `Save ${item.title}`} className={`inline-flex items-center gap-2 text-sm font-black ${saved.includes(item.id) ? "text-[var(--leaf-dark)]" : "text-[var(--muted)] hover:text-[var(--ink)]"}`}><Icon name="heart" /> {saved.includes(item.id) ? "Saved" : "Save"}</button><button onClick={() => shareItem(item)} className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-black text-[var(--muted)] hover:bg-[#f1eee7] hover:text-[var(--ink)]"><Icon name="share" /> Share</button></div></article>)}</div> : <div className="mt-5 rounded-[26px] border border-dashed border-[var(--line)] bg-white/70 p-10 text-center"><div className="text-4xl" aria-hidden="true">🔎</div><h3 className="mt-3 text-xl font-black">Nothing matches that yet.</h3><p className="mt-2 text-sm text-[var(--muted)]">Try a different search, category or saved filter.</p><button onClick={() => { setQuery(""); setCategory("All"); setShowSavedOnly(false); }} className="mt-5 rounded-full bg-[var(--ink)] px-5 py-2.5 text-sm font-black text-white">Clear filters</button></div>}</div></section>
